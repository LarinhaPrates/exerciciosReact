...
function EditarEscola() {
  const { id } = useParams(); // id da escola
  ...
  const [erroEscola, setErroEscola] = useState(null);
  ...
  useEffect(() => {
    const carregarEscola = async () => {
      if (!id) {
        setErroEscola('ID da escola não fornecido.');
      ...
      try {
        ...
        setFormData({
          ...
          cidade: data.cidade || '',
          administradorResponsavel: data.admResponsavel || ''
        });
      } catch (e) {
        // eslint-disable-next-line no-console
        console.warn('Falha ao carregar escola:', e);
        setErroEscola('Não foi possível carregar dados da escola.');
      } finally {
        setCarregandoEscola(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error) throw error;
      if (status === 204) {
        setSucessoSalvar('Escola atualizada com sucesso!');
      } else {
        setSucessoSalvar('Atualização concluída.');
      }
    } catch (err) {
      // eslint-disable-next-line no-console
      ...
      <main className="flex items-center justify-center py-12 px-4">
        <div className="bg-white rounded-3xl shadow-xl p-12 w-full max-w-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {carregandoEscola && (
              <p className="text-center text-gray-600">Carregando dados da escola...</p>
            )}
            {erroEscola && (
              <p className="text-center text-red-600">{erroEscola}</p>
            )}
            {/* Nome da Escola */}
            <div>
